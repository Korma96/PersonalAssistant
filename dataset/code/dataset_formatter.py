...
def _parse_line(line: str, num_of_columns: int, intent_config: dict, all_slots: list, all_requests: list, invalid_data: list):
    ...
    if intent_name in intent_config.keys():
            ...
            for slot in slots:
                ...
                if slot_name in intent_config[intent_name][const.ALL_SLOTS]:
                    ...
                    if helpers.contains_only_numbers(slot_value):
                        invalid_data.append(InvalidData(intent=intent_name, request=request, slot_name=slot_name,
                                                        slot_value=slot_value, message='Slot value is number'))
        ...
        if request and should_add_request and helpers.is_request_valid(request, intent_name):
            all_requests.append((intent_name, request))
        else:
            invalid_data.append(InvalidData(intent=intent_name, request=request, message='Invalid request', slot_value=slots_csv))
